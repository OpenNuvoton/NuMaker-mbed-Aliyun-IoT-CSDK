...
#if defined(__MBED__)
    mbedtls_net_context_alt fd;       /**< mbed TLS network context. */
#else
...
#define SEED_MAGIC 0x123
    ...
    static const char *g_seed_key = "seed_key";
...
#if defined(__MBED__)
                            mbedtls_net_context_alt *tcp_fd,
#else
...
#else
#if defined(__MBED__)
    if (0 != (ret = mbedtls_net_connect_alt(&(pTlsData->fd), addr, port, MBEDTLS_NET_PROTO_TCP))) {
#else
...
#if defined(MBEDTLS_SSL_MAX_FRAGMENT_LENGTH) && (CONFIG_MBEDTLS_MFL_CODE > 0)
    if ((ret = mbedtls_ssl_conf_max_frag_len(&(pTlsData->conf), CONFIG_MBEDTLS_MFL_CODE)) != 0) {
        printf("SSL fragment setup error code -0x%04x\n", -ret);
        return ret;
    }
#endif
#endif
...
#endif /* #if (TLS_AUTH_MODE == TLS_AUTH_MODE_CA) */
#if defined(__MBED__)
    mbedtls_ssl_set_bio(&(pTlsData->ssl), &(pTlsData->fd), mbedtls_net_send_alt, mbedtls_net_recv_alt, mbedtls_net_recv_timeout_alt);
#else
...
static int _network_ssl_read(TLSDataParams_t *pTlsData, char *buffer, int len, int timeout_ms)
{
    ...
    /* NOTE: We don't support non-blocking read (timeout_ms == 0?). See mbedtls_ssl_set_bio(...)/
     *       mbedtls_ssl_conf_read_timeout(...) for details. */
#endif
...
#if defined(__MBED__)
    /* TODO: Check timeout 0 in ali means non-blocking or blocking. It is not well-defined. */
    mbedtls_net_set_mbedtimeout(&(pTlsData->fd), cast_timeout_ali2mbed(timeout_ms));
#endif
...
#if defined(__MBED__)
    mbedtls_net_free_alt(&(pTlsData->fd));
#else
                            ...
                            uint32_t ca_crt_len)
{
    ...
    pTlsData = (TLSDataParams_pt) g_ssl_hooks.malloc(sizeof(TLSDataParams_t));
    if (NULL == pTlsData) {
